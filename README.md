...
подписки Claude. На текущем датасете (4 синтетических + 12 из реальной практики ФАС)
baseline — **12/16 PASS**; failure-кейсы из реальных решений ФАС используются как
сигнал для тюнинга промптов чекеров. Per-case инфраструктурные сбои eval больше не
обрушивают весь прогон — каждый кейс изолирован и помечается `ERROR` отдельно.
...
case_law/
  fas_decisions/             # 12 решений ФАС / УФАС / арбитража с permalink, цитатой рекламы, mapped RuleId
  regression_dataset/        # JSONL для `make eval` — вход + ожидаемые нарушения
    example.jsonl            #   4 синтетических (recall + precision sanity)
    fas_practice.jsonl       #   12 distilled из реальной практики ФАС 2017–2024
  approved_reports/          # отчёты, утверждённые пользователем в UI — корпус под будущий RAG
  research/                  # сырые research-прогоны (Perplexity Deep Research, актуальная база)
...
## 🧠 База кейсов ФАС и обучение агента

Агент не «обёртка над generic safety prompt» — он привязан к реальной правоприменительной
практике. На сегодня в репозиторий внесены **12 кейсов из практики ФАС России / УФАС /
арбитражных судов 2017–2024 гг.** ([case_law/fas_decisions/](case_law/fas_decisions/)),
плюс 4 синтетических теста — итого 16 строк в регрессионном датасете.

### Откуда берутся кейсы — пайплайн сбора

1. **Research-прогон** ([Perplexity Deep Research](case_law/research/) или ручной обход
   `fas.gov.ru` / `kad.arbitr.ru`) → сохраняется в `case_law/research/<дата>.md`
   как первичный источник: дата сбора, количество найденных кейсов, ссылки, executive
   summary, разделы «Пробелы данных».
2. **Дистилляция в per-decision MD** ([case_law/fas_decisions/](case_law/fas_decisions/)).
   Каждый кейс с дословной цитатой рекламы → отдельный `<YYYY-MM-DD>-<slug>.md` по
   шаблону [`case_law/README.md`](case_law/README.md): permalink ФАС, орган, рекламодатель,
   класс препарата (Rx / OTC / БАД), канал размещения, дословная цитата, нарушенная норма,
   маппинг на `RuleId` enum, штраф.
3. **Регрессионный JSONL** ([case_law/regression_dataset/fas_practice.jsonl](case_law/regression_dataset/fas_practice.jsonl)).
   Каждый кейс с пригодной цитатой → строка `{id, input, expected_rule_ids, source}`.
   `expected_rule_ids` валидируется по `RuleId` enum (если нарушение не покрыто текущими
   чекерами — кейс в JSONL не идёт, а фиксируется в разделе «Пробелы» research-файла).
   Дополнительно для каждого нарушения добавляется **paired clean control** — тот же
   препарат без триггерных фраз и с правильным дисклеймером — для проверки precision.

### Цикл «Утвердить ответ» → обогащение базы знаний

В Streamlit-UI после каждой проверки доступна кнопка **«Нормальный ответ»** (секция
«✅ Утвердить ответ»). Нажатие сохраняет в [`case_law/approved_reports/`](case_law/approved_reports/)
JSON со всей структурой `ComplianceReport`:

- **входной креатив** (текст / распознанный из картинки / URL / PDF),
- **класс препарата** (определённый классификатором),
- **найденные нарушения** с цитатами и предложениями правки,
- **переписанный compliant-вариант**,
- **история комментариев пользователя** из итераций (`user_feedback`).

Имя файла — `approved-<UTC-timestamp>-<sha>.json`. Текущий счётчик утверждённых отчётов
отображается в сайдбаре (`📚 База знаний: N утверждённых отчётов`). Это и есть **источник
истины «как должен выглядеть валидированный ответ»**: то, что пользователь подтвердил
после, возможно, нескольких итераций с комментариями, становится эталоном.

### Планируется: настоящий RAG

Текущий [`rag/law_index.py`](src/pharma_ad_compliance/rag/law_index.py) — это **v0.1**:
просто кэшированное чтение текста ст. 24 ФЗ-38. Следующий этап — **векторный индекс
по `case_law/fas_decisions/` + `approved_reports/`**:

- эмбеддинги per-decision MD + утверждённых отчётов кладутся в локальный store (например, FAISS / Chroma);
- при запуске пайплайна по каждому чекеру делается top-K retrieval по входному креативу;
- релевантные кейсы (с реальной цитатой ФАС и обоснованием) подмешиваются в системный промпт чекера как few-shot;
- агент перестаёт работать «вслепую по статье 24» и начинает оперировать в контексте
  накопленной практики ФАС и собственных подтверждённых ответов.

Структура case_law уже приведена к стабильной схеме под это: per-decision MD имеют единый
формат с mapped RuleId, JSONL валидируется по enum, `approved_reports/*.json` имеют тот
же Pydantic-schema что и `ComplianceReport` — повторный парсинг не нужен.

## 📚 Источники

- [ФЗ-38 «О рекламе» статья 24](http://www.consultant.ru/document/cons_doc_LAW_58968/) — текст закона.
- [Решения ФАС по рекламе ЛС](https://fas.gov.ru/) — публичная база, основа `case_law/fas_decisions/`.
- Свежий research-прогон с обзором практики 2018–2026 — [`case_law/research/perplexity-2026-05-25.md`](case_law/research/perplexity-2026-05-25.md).

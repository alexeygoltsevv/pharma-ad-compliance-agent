...
Дополнительное поле `intent_hypothesis` (1 предложение, на русском):
сформулируй, что бренд, скорее всего, ПЫТАЛСЯ сказать этой формулировкой —
какую коммерческую задачу решал. Это не оправдание нарушения, это понимание
замысла. Примеры формата:
- «Бренд хотел дифференцироваться через эффективность — переборщил с гарантией».
- «Бренд снимал страх побочки — дал абсолютное утверждение».
- «Бренд цитировал реальный отзыв — не учёл п. 2 о случаях излечения».

Поле обязательное для каждой violation. Если намерение неочевидно — напиши
«Замысел неочевиден из контекста». Не повторяй текст `explanation`.
  ...
  "violations": [
    {
      ...
      "intent_hypothesis": "<1 предложение: что бренд пытался сказать>"
    }
  ]
}

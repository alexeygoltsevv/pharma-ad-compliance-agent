...
async def run_compliance(creative: Creative, *, include_rewrite: bool = True) -> ComplianceReport:
    parsed = await parser_agent.parse(creative)
    drug_class = await drug_classifier_agent.classify(parsed)

    checker_runs = [checker(parsed, drug_class) for checker in ALL_CHECKERS]
    ...
    rewritten = (
        await editor_agent.rewrite(parsed=parsed, drug_class=drug_class, violations=violations)
        if include_rewrite
        else None
    )
